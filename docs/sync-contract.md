...
| --- | --- |
| Any `TIER1_RULE_CODE` value — `VALUE_NOT_NUMERIC`, `VALUE_NOT_POSITIVE`, `VALUE_EXCEEDS_MAX_MAGNITUDE`, `VALUE_EXCEEDS_MAX_PRECISION`, `METHOD_REQUIRED`, `METHOD_NOT_APPLICABLE`, `EFFECTIVE_DATE_TIME_IN_FUTURE`, `EFFECTIVE_DATE_TIME_BEFORE_SURGERY` | Server-side re-enforcement of `packages/core`'s Tier 1 rules (AC 13.1 AC3). Returned verbatim, not remapped. `VALUE_EXCEEDS_MAX_MAGNITUDE` and `VALUE_EXCEEDS_MAX_PRECISION` (P2.S1a) are the canonical column's representability bounds — a value at or beyond `10^8`, or carrying more than 4 decimal places. Both were previously an HTTP `500`, which §9 tells a client to re-push indefinitely; as Tier 1 rejections they are correctable instead. Neither is clinical: implausible-but-real magnitudes stay Tier 2 and are never rejected. `METHOD_NOT_APPLICABLE` (P3.S2) is `METHOD_REQUIRED`'s mirror for an entry carrying NO volume — a colour-only voided-urine entry (AC 12.1 AC2) has no number for Measured/Estimated to describe. Without it the contradiction reaches the database CHECK and returns a `500`, which §9 tells a client to re-push indefinitely; as a Tier 1 rejection it reaches the correction inbox instead. **It fires on an EXPLICIT qualifier only.** Wire `null` on a volume-less payload means "no toggle applies" and is accepted — see §7.2's `method` row, which is where that reading is defined. Rejecting it would refuse the only payload a client can construct for the case, since §7.2 forbids omitting the field. |
...
| `code` | FHIR | yes | Bare LOINC code. P3.S2 accepts `79560-9` (stoma output), `9000-1` (oral fluid intake) and `9187-6` (voided urine); other codes are `UNSUPPORTED_CODE` until their sprint lands. Widening the accepted set is additive under §8. |
| `valueQuantity.value` | FHIR | **conditional** | Required on every code EXCEPT `9187-6`, where it may be omitted entirely if `urineColorCode` is present (AC 12.1 AC2 — a patient who cannot measure records a colour instead). **Omitted, never sent as `0` or `null`:** a missing volume is not a void of zero, and a consumer that coerces one to the other produces a wrong daily total and raises nothing. Omitting it on any other code is `PAYLOAD_FIELD_INVALID`. **Canonical units always** — mL for volume, kg for weight (ADR-0004). The client converts before sending; the server never receives ounces. A decimal, not an integer (ADR-0005). |
| `valueQuantity.unit` | FHIR | **conditional** | Present exactly when `valueQuantity.value` is — the two travel together in both directions, enforced by a database CHECK as well as here. `"mL"` or `"kg"`. Determined by `code`, transmitted anyway because FHIR requires it and a stored unit makes a future misreading recoverable rather than guessed at. |
| `effectiveDateTime` | FHIR | yes | RFC 3339, UTC, millisecond precision. The clinical moment — see §1. |
| `method` | FHIR | yes, nullable | SNOMED CT `414135002` \|Estimated (qualifier value)\| when estimated, `258104002` \|Measured (qualifier value)\| when measured (ADR-0018, amended). Explicitly present, never omitted: the mandatory Measured/Estimated selection is Tier 1, and an absent key cannot be told apart from a client that does not implement the toggle. **`null` is read against `valueQuantity`, which is the only thing that disambiguates it.** On a payload that *carries* a volume, `null` still means measured — a client built before this amendment sends it and §8 requires the server to keep understanding an older client — and is normalised to `258104002` before storage, so no row records the ambiguity. On a payload carrying **no** volume (a colour-only `9187-6` entry, AC 12.1 AC2) there is nothing for the toggle to describe: `null` means "no toggle applies" and is stored as SQL `NULL`. That is the **only** way a client can say so, because this field may not be omitted — which is why reading it as measured there made every colour-only entry from a device fail `METHOD_NOT_APPLICABLE`, in a correction inbox showing no toggle to fix (§9.2's retry-unchanged loop). An **explicit** `258104002` or `414135002` on a volume-less payload is still `METHOD_NOT_APPLICABLE`: a client asserting a technique for a number it did not supply is a different thing from a client with no way to stay silent. `null` at rest therefore means only "this observation has no Measured/Estimated toggle" — a colour-only voided-urine entry today, weight and resting heart rate later. Every other non-null value is `PAYLOAD_FIELD_INVALID`. |
...
| `fluidTypeCode` | app-native | no | **Added at P3.S1** (SRS AC 2.3 AC1) — a new *optional* payload field, which §8 makes additive: an older client that never sends it keeps working. Which kind of fluid this was, as a `fluid_type` value-set member **code**, never a display label (ADR-0006). Meaningful only on `9000-1`; sending it with any other code is `PAYLOAD_FIELD_INVALID`, because nothing would ever read it and a value no read path interprets is worse than an absent one. Optional even on intake, so `null` means "the patient did not say". The server does **not** check membership in the value set: members are retired-never-deleted and admin-managed, so validating against the live set would refuse a patient's entry the moment an admin retired a member a fielded app still offers — an unknown code renders as a generic label and is recoverable, a refused entry is not. |
| `urineColorCode` | app-native | no | **Added at P3.S2** (SRS AC 12.1 AC2) — a new *optional* payload field, additive under §8 exactly as `fluidTypeCode` was. The chosen step of the pale-to-dark urine colour scale, as a `urine_color` value-set member **code**, never a display label (ADR-0006). Meaningful only on `9187-6`; sending it with any other code is `PAYLOAD_FIELD_INVALID`. It is the only field a colour-without-volume entry carries, so a `9187-6` payload with **neither** `valueQuantity` nor this is refused — it would record nothing. The server does **not** check membership in the value set, for the same reason given for `fluidTypeCode`: an unknown code renders as a generic label and is recoverable, a refused entry is not. **The colour scale has no standard terminology and deliberately no invented one** — it is an app-native value set, and until a code system URI is decided the FHIR export module must not emit it as a coded component. |
...
**The SNOMED CT estimation-technique code (D4) — RESOLVED.** `414135002` \|Estimated (qualifier value)\|, per ADR-0018. `packages/core`'s `ESTIMATION_METHOD_CODE` publishes it and stays a discriminated union, so the shape is there for the next unresolved terminology code (voided-urine colour at P3.S2, the resting-conditions flag at P7). **Changing the value later is a data migration, not an edit** — rows already written keep the old code and nothing detects the disagreement until FHIR export.

**Whether `method: null` should become an explicit |Measured| code — RESOLVED, adopted.** `258104002` \|Measured (qualifier value)\| is now written for every volumetric entry (ADR-0018, amended). `null` previously carried two meanings distinguished only by `code` — "volumetric, measured" and "weight entry, toggle does not apply" — an inference that was sound inside this system and invisible outside it, since an exported `Observation` with no `method` says *not stated*, not *measured*. The distinction is now data. `null` on the wire remains accepted and still means measured, for the §8 reason given in §7.2; it is normalised before storage.
