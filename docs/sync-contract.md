...
| `method` | FHIR | yes, nullable | SNOMED CT `414135002` \|Estimated (qualifier value)\| when estimated, `258104002` \|Measured (qualifier value)\| when measured (ADR-0018, amended). Explicitly present, never omitted: the mandatory Measured/Estimated selection is Tier 1, and an absent key cannot be told apart from a client that does not implement the toggle. `null` is still **accepted** and still means measured, because a client built before this amendment sends it and §8 requires the server to keep understanding an older client — but the server normalises it to `258104002` before storing, so no row records the ambiguity. `null` at rest now means only "this observation has no Measured/Estimated toggle" (weight, resting heart rate). Every other non-null value is `PAYLOAD_FIELD_INVALID`. |
