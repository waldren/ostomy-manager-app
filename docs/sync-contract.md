...
| `method` | FHIR | yes, nullable | SNOMED CT `414135002` \|Estimated (qualifier value)\| when estimated; `null` when measured. Explicitly `null`, never omitted: the mandatory Measured/Estimated selection is Tier 1, and an absent key cannot be told apart from a client that does not implement the toggle. **D4 is resolved** (ADR-0018) and `packages/core` publishes the code; the server accepts `null` and that one code, and rejects **every** other non-null value with `PAYLOAD_FIELD_INVALID` — including SNOMED `258104002` \|Measured (qualifier value)\|, which is a real concept this wire deliberately does not use. See ADR-0018 for why `null` remains the measured representation, and what it costs. |
...
**Whether `method: null` should become an explicit |Measured| code — open.** `258104002` \|Measured (qualifier value)\| exists. Today `null` carries two different meanings distinguished only by `code`: "volumetric entry, measured" and "weight entry, the toggle does not apply". That inference is sound inside this system and invisible outside it — an exported `Observation` with no `method` says *not stated*, not *measured*. Adopting the code makes the distinction data rather than inference, and is free now and a versioned §8 change once clients ship. ADR-0018 records the argument; the decision is not made.
