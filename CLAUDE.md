...
Seventeen are accepted (ADR-0001 to ADR-0017), covering the sync wire contract, testing strategy, monorepo tooling, canonical units, entry precision, i18n, `packages/core` ownership, admin API sequencing, seed data, the API's CommonJS module system, database roles and audit immutability, entered-measurement-system provenance, the delta cursor's visibility mechanism, on-device PHI encryption and ownership, biometric local access, the patient-local day boundary, and PHI retention/deletion. ADR-0001 (sync) and ADR-0007 (`packages/core` ownership) bind almost every sprint — read both before writing shared or sync code.
