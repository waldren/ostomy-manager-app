...
Audit logging exists and is real (P1.S5): a **global** `AuditInterceptor`, an `@Audited()` decorator, and `route-guard-coverage.spec.ts`, which fails the build if any mutating route carries no `@Audited()`. `POST`/`GET /api/v1/observations` (stoma output only) is the first business-logic surface (P2.S1a) — it writes the observation and its audit row **in one transaction**, and is the shape every later PHI write path should copy rather than re-derive. Sync (`/api/v1/sync/push`, `/api/v1/sync/delta`) is P2.S1b and does not exist yet. See `apps/api/README.md`.
